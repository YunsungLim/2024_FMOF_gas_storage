...
class Locator:
    ...
    def locate_with_permutation(self, target, bb, permutation, rod_connection_index):
        """
        ...
        # prepare sub permutation
        if len(rod_connection_index) != 0:
            sub_permutation = permutation.copy()
            i1 = np.where(np.array(permutation) == rod_connection_index[0])[0][0]
            i2 = np.where(np.array(permutation) == rod_connection_index[1])[0][0]
            sub_permutation[i1], sub_permutation[i2] = sub_permutation[i2], sub_permutation[i1]
        ...
        # Rotation matrix.
        U, rmsd_val = find_best_orientation(p_coord, q_coord)
        updated_permutation = permutation
        
        # arbitrary value for cutoff
        if (len(rod_connection_index) != 0) and (rmsd_val > 0.5):
            q_coord = q_coord[sub_permutation]
            U, rmsd_val = find_best_orientation(p_coord, q_coord)
            # save new permutation
            updated_permutation = sub_permutation
        ...
        return bb, rmsd_val, updated_permutation
